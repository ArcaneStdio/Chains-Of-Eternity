...
public class ProjectileSpellRuntime : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D other)
    {
        ...
        if (((1 << other.gameObject.layer) & data.targetLayerMask) != 0)
        {
            ...
            hitCount++;
            if (data.destroyOnHit || hitCount >= data.maxPierceCount)
                DestroyProjectile();
        }
    }
    ...
    public void Initialize(ProjectileData d, Transform c, Vector2 Direction)
    {
        if (d == null)
        {
            Debug.LogError("ProjectileData null");
            return;
        }
        ...
        if (enableDebugLogs)
            Debug.Log($"Projectile launched toward mouse. Speed: {data.projectileSpeed}");
    }
    ...
    private IMovementPattern CreateMovementPattern() =>
        data.movementPath switch
        {
    ...
    private void DestroyProjectile()
    {
        if (data.hitEffectPrefab)
            Instantiate(data.hitEffectPrefab, transform.position, transform.rotation);
        if (data.hitSound)
    ...
    private void OnDrawGizmos()
    {
        ...
        Gizmos.DrawLine(
            transform.position,
            transform.position + (Vector3)GetCurrentVelocity() * 0.1f
        );
    }
}
...
{
    void Initialize(ProjectileSpellRuntime p, ProjectileData d, Vector2 dir);
...
public class ArcMovementPattern : IMovementPattern
{
    ...
    public void Initialize(ProjectileSpellRuntime p, ProjectileData d, Vector2 dir)
    {
        ...
        if (rb != null)
            rb.gravityScale = d.arcGravityScale;
...
public class HomingMovementPattern : IMovementPattern
{
    ...
    public void UpdateMovement(float dt)
    {
        ...
        if (d.rotateToFaceDirection)
            p.transform.rotation = Quaternion.AngleAxis(
